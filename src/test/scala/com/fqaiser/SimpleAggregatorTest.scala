...
class SimpleAggregatorTest extends AnyFeatureSpec with Matchers {
  ...
  Feature("only one type of animal in a given zoo") {
    ...
    Scenario("1 animal created and then updated") {
      runTest { (testDriver, inputTopic, outputTopic) =>
        val animalKey = AnimalKey(animalId1)
        val animalValue1 = AnimalValue(animalId1, zooId, maxCalories)
        val animalValue2 =
          animalValue1.copy(maxCalories = animalValue1.maxCalories + 10)
  ...
  Feature("multiple types of animals in a given zoo") {
    Scenario("2 animal create events") {
      runTest { (testDriver, inputTopic, outputTopic) =>
        val animalValue1 = AnimalValue(animalId1, zooId, maxCalories)
        val animalValue2 = AnimalValue(animalId2, zooId, maxCalories)
        ...
        val expected = Seq(
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animalValue1))
          ),
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animalValue1, animalValue2))
          )
        )
    ...
    Scenario("2 animal create events and 1 animal is updated") {
      runTest { (testDriver, inputTopic, outputTopic) =>
        val animal1Value = AnimalValue(animalId1, zooId, maxCalories)
        val animal2Value = AnimalValue(animalId2, zooId, maxCalories)
        val animal2Value2 =
          animal2Value.copy(maxCalories = animal2Value.maxCalories + 10)
        ...
        val expected = Seq(
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animal1Value))
          ),
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animal1Value, animal2Value))
          ),
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animal1Value))
          ...
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animal1Value, animal2Value2))
          )
        )
    ...
    Scenario(
      "2 animal create events and 1 animal is updated and the other animal is deleted"
    ) {
      runTest { (testDriver, inputTopic, outputTopic) =>
        val animal1Value = AnimalValue(animalId1, zooId, maxCalories)
        val animal2Value = AnimalValue(animalId2, zooId, maxCalories)
        ...
        val expected = Seq(
          ...
          new KeyValue(
            ZooAnimalsKey(zooId),
            ZooAnimalsValue(List(animal2Value2))
          )
        )
